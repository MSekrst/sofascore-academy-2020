...
# React Reusability & Separation

- Often some logic can be reused or separated to new file
- Separate logic and presentation in different places
- Insert logic into component from other component (or function)
- React patterns (Decorator Design pattern)
...
- Separation of concerns is important principe in software development
- React allows us to insert logic into component via common React patterns (which are in fact Decorator design pattern):
...
- Passes handler or value to component via props
  - Problem when passed prop is already in use by component (in `this.props`)
...
- Component without conventional render method
  - Receives render method as prop
...
function Component(props) => {
  return <RenderPropComponent render={enhancements => /* do something with enhancements */} />
})
```
...
- More declerative usage then HOC, as it is used inside component that is decorated, so prop name colision is fixed.
...
- Cleanest syntax, easier to understand
...
- Share data bewtween components
  - Data is passed from parent (`Provder`) to children (`Consumer`)
...
- React Context allows developers to share date between parent and mutiple children. Parent is called `Provider` as it provides value for children.
...
- Context is crated with `createContext` function. Function receives optional defaultValue which will be used as initial context value.
  Context contains two components (that are used from the outside):
  - Provider (`MyContext.Provider`)
  - Consumer (`MyContext.Consumer`)
- Contexts are usualy added to separate file and they export `Context` itself, `Provider` and `Consumer` components.
...
- Event propagation and Context work as it is still in hierarchy
...
- Singleton pattern is common as apps usualy have only one element over others.
