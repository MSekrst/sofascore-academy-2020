...
- React methods to fix common problems:
  - Wrapping elements
  - Prop drilling
  - Reuse logic
...
- Prop drilling -> Anti-pattern where props are passed through component tree (e.g. parent sends handler deeper down tree).
...
- Can be used to wrap element in single element
- Won't add top level element (Fragment) to the DOM
- Can't have props (even `key`)

<Notes>

- Commonly used in React codebases.
- Can't take props (event `key`) so it should be avoided as top element for list items, where `key` is necessary.
...
- wrapping in `div` kills hierarchy, fragment fixes
...
- get reference on underlying DOM element
- acess DOM element properties (height, position, ...)
- `useRef` hook
...
- get element height for generic accordion
...
# High Order Component / Render Props
