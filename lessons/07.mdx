...
- Applications are more complex then ever (especially Frontend part)
- Handle various data (fetch responses, cached data, loaders, global data, ...)
...
- Applications have become mix of two different concepts: Mutation and Asynchronicity. Popular example for them is Mentos and Coke.
  They are great when separated but messy when combined. React is solves asynchronicity from view layer but state mutations are still preset.
...
- Plain React hook that tries to imitate Redux pattern
...
- `useReducer` replaces `useState` hook
...
- `useReducer` can supstitute multiple `useState` hooks
